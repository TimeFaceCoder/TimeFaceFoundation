...
- (BOOL)isEmojized {
    ...
    NSRange matchingRange = NSMakeRange(0, [self length]);
    NSArray *array = [regex matchesInString:self options:NSMatchingReportCompletion range:matchingRange];
    if (array.count) {
        has = YES;
    }
    return has;
...
- (BOOL)hasEmojize {
    return [self isEmojized];
}

- (void)setHasEmojize:(BOOL)hasEmojize {
    self.hasEmojize = hasEmojize;
}
