//
//  TFCameraCollectionViewFlowLayout.m
//  TimeFaceV2
//
...
@interface TFCameraCollectionViewFlowLayout()
...
@implementation TFCameraCollectionViewFlowLayout
@dynamic scrollDirection;
-(id) init
{
    self = [super init];
    if ( self ) {
        CGFloat size = (CGRectGetWidth(TTScreenBounds()) - 40 ) / 3;
...
-(void)prepareLayout {
    ...
    NSArray *noLongerVisibleBehaviours = [self.dynamicAnimator.behaviors filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(UIAttachmentBehavior *behaviour, NSDictionary *bindings) {
        BOOL currentlyVisible = [itemsIndexPathsInVisibleRectSet member:[[[behaviour items] firstObject] performSelector:@selector(indexPath)]] != nil;
        return !currentlyVisible;
    }]];
    
    [noLongerVisibleBehaviours enumerateObjectsUsingBlock:^(id obj, NSUInteger index, BOOL *stop) {
        [self.dynamicAnimator removeBehavior:obj];
        [self.visibleIndexPathsSet removeObject:[[[obj items] firstObject] performSelector:@selector(indexPath)]];
    }];
...
-(BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    ...
    [self.dynamicAnimator.behaviors enumerateObjectsUsingBlock:^(UIAttachmentBehavior *springBehaviour, NSUInteger idx, BOOL *stop) {
        ...
        UICollectionViewLayoutAttributes *item = [springBehaviour.items performSelector:@selector(firstObject)];
        CGPoint center = item.center;
        if (delta < 0) {
