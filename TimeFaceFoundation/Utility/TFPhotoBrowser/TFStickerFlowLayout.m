...
-(NSString *)description{
    return [NSString stringWithFormat:@"MatrixElemnt %@x%@ for indexPath: %@ - %@, frame: %@", @(self.columns), @(self.rows), @(self.indexPath.item), @(self.indexPath.section), NSStringFromCGRect(self.frame)];
}

@end

@interface TFStickerFlowLayout()
...
@implementation TFStickerFlowLayout
...
-(void) printMatrix:(NSMutableArray *) matrix{
    ...
    for(NSInteger i = 0; i < matrix.count; i++){
        ...
        for(NSInteger j = 0; j < row.count; j++){
            ...
            description = [description stringByAppendingFormat:@"\t %@", @(element.indexPath.item)];
        }
        description = [description stringByAppendingString:@"\n"];
    }
    TFLog(@"Matrix: %@", description);
}
