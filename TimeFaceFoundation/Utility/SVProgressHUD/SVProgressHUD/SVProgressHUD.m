...
+ (void)showGifWithStatus:(NSString *)status {
    
    NSMutableArray *loadingImgs = [NSMutableArray arrayWithCapacity:1];
    for (int i  = 1; i <= 7; i++) {
        [loadingImgs addObject:[UIImage imageNamed:[NSString stringWithFormat:@"HUDLoading%d.png",i]]];
    }
    
    [[self sharedView] showProgress:-1 images:loadingImgs status:status maskType:SVProgressHUDMaskTypeClear];
}
...
- (void)updatePosition {
    ...
    BOOL imageUsed = (self.imageView.animationImages) || (self.imageView.image) || (self.imageView.hidden);
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    CGFloat posX = orientationFrame.size.width/2 - 50;
...
- (void)showProgress:(float)progress
    ...
    if (images) {
        ...
        self.imageView.animationDuration = (CGFloat)images.count/12.0;
        self.imageView.hidden = NO;
